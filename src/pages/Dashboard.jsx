  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        ...
        <p className="text-slate-400 font-medium">Here's a premium diagnostic look at your career path today.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2">
          <GlassCard className="h-96 flex flex-col">
            <h3 className="text-lg font-semibold text-white mb-4">Application Engagement Rate</h3>
            {/* Visual engagement analytics bar representation */}
            <div className="flex-1 border border-white/5 rounded-xl bg-black/20 flex flex-col justify-between p-6">
              ...
              <div className="h-4 bg-white/5 rounded-full overflow-hidden flex border border-white/10 my-4">
              ...
              {safeApps.length > 0 ? safeApps.map(app => {
                ...
                <div key={app.id} className="p-3 bg-white/5 border border-white/5 rounded-xl flex items-center justify-between hover:border-white/10 transition-colors">
                  <div className="truncate pr-2">
                    ...
                    <div className="text-[10px] text-slate-400 truncate">{app.Job?.company}</div>
                  </div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="lg:col-span-1 space-y-4">
        <GlassCard className="h-full relative overflow-hidden border-t-4 border-[#0ea5e9]">
          ...
          <p className="text-xs text-slate-400 mb-4 font-medium">Active jobs matching your profile database skills.</p>

          <div className="space-y-3">
            {recommendations.length > 0 ? recommendations.map(job => (
              <div key={job.id} className="p-3 bg-white/5 border border-white/5 rounded-xl hover:border-[#0ea5e9]/40 transition-all">
                <h4 className="text-xs font-bold text-white mb-0.5">{job.title}</h4>
                <p className="text-[10px] text-[#0ea5e9] font-medium mb-2">{job.company} • {job.location}</p>
                <div className="flex flex-wrap gap-1">
                  {job.tags.map(tag => (
                    <span key={tag} className="text-[8px] px-1.5 py-0.5 bg-black/40 border border-white/5 text-slate-400 rounded">
                      {tag}
                    </span>
                  ))}
                </div>
      ...
      <div className="lg:col-span-1 space-y-4">
        <GlassCard className="h-full relative overflow-hidden border-t-4 border-[#8b5cf6]">
          ...
          <p className="text-xs text-slate-400 mb-4 font-medium">Trending keywords in postings that you haven't listed yet.</p>

          <div className="space-y-4">
            {gaps.length > 0 ? gaps.map((gap, index) => (
              <div key={gap} className="space-y-1">
                <div className="flex justify-between text-xs font-medium">
                  <span className="text-slate-300 capitalize">{gap}</span>
      ...
      <div className="lg:col-span-1 space-y-4">
        <GlassCard className="h-full relative overflow-hidden border-t-4 border-[#10b981]">
          ...
          <p className="text-xs text-slate-400 mb-4 font-medium">Official materials and interview diagnostics compiled by admins.</p>
...
const StatCard = ({ title, value, trend, icon: Icon, color }) => (
  <GlassCard className="flex flex-col gap-2 relative overflow-hidden">
    ...
    <div className="text-[10px] text-slate-500 mt-1">{trend}</div>
    <div 
